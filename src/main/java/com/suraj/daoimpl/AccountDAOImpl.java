...
import com.suraj.dao.AccountDAO;
...
public class AccountDAOImpl implements AccountDAO {
    ...
    private final static String SQL_CREATE_ACC = "INSERT INTO Account (CustomerName, Balance, CurrencyCode) VALUES (?, ?, ?)";
    ...
    public Account getAccountById(long accountId) throws CustomException {
        ...
        try {
            ...
            rs = stmt.executeQuery();
            if (rs.next()) {
                acc = new Account(rs.getLong("AccountId"), rs.getString("CustomerName"), rs.getBigDecimal("Balance"),
    ...
    public int updateAccountBalance(long accountId, BigDecimal deltaAmount) throws CustomException {
        ...
        try {
            ...
            if (balance.compareTo(CurrencyUtil.zeroAmount) < 0) {
     ...
     */
    public int transferAccountBalance(CustomerTransaction customerTransaction) throws CustomException {
        int result = -1;
        Connection conn = null;
        PreparedStatement lockStmt = null;
        PreparedStatement updateStmt = null;
        ...
        try {
            ...
            rs = lockStmt.executeQuery();
            if (rs.next()) {
                toAccount = new Account(rs.getLong("AccountId"), rs.getString("CustomerName"), rs.getBigDecimal("Balance"),
                        rs.getString("CurrencyCode"));
                if (log.isDebugEnabled())
            ...
            if (!fromAccount.getCurrencyCode().equals(customerTransaction.getCurrencyCode())) {
            ...
            BigDecimal fromAccountLeftOver = fromAccount.getBalance().subtract(customerTransaction.getAmount());
            if (fromAccountLeftOver.compareTo(CurrencyUtil.zeroAmount) < 0) {
            ...
            updateStmt.addBatch();
            updateStmt.setBigDecimal(1, toAccount.getBalance().add(customerTransaction.getAmount()));
